...
fn filter_binaries<'a>(binaries: &Vec<&'a str>, idx: usize, chosen_digit: char) -> Vec<&'a str> {
    binaries
        .iter()
        .copied()
...
fn calculate_ogr<'a, 'r>(binaries: &'r Vec<&'a str>, idx: usize) -> Vec<&'a str> {
    if binaries.len() <= 1 {
        return binaries.to_owned();
    }
...
fn calculate_csr<'a, 'r>(binaries: &'r Vec<&'a str>, idx: usize) -> Vec<&'a str> {
    if binaries.len() <= 1 {
        return binaries.to_owned();
    }
    let (zeros, ones) = count_occurrences(binaries, idx);
...
pub fn second_task(content: &String) -> u32 {
    const INITIAL_IDX: usize = 0;
    let binaries: Vec<&str> = content.lines().collect();
    let ogr = (isize::from_str_radix(calculate_ogr(&binaries, INITIAL_IDX)[0], 2).unwrap()) as u32;
    let csr = (isize::from_str_radix(calculate_csr(&binaries, INITIAL_IDX)[0], 2).unwrap()) as u32;
    ogr * csr
}
